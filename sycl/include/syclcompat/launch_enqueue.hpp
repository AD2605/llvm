...
template <auto KernelFunctor, typename PropertyList, typename... Args>
inline std::enable_if_t<
    std::is_invocable_v<decltype(KernelFunctor), Args..., char *> &&
...
launch(const sycl::nd_range<3> &launch_params, std::size_t local_memory_size,
  ...
  return sycl_exp::submit_with_event(queue, [&](sycl::handler &cgh) {
    ...
    sycl_exp::nd_launch(cgh, config, [=](sycl::nd_item<3> it) {
      KernelFunctor(args...,
                    local_memory.get_multi_ptr<sycl::access::decorated::yes>());
    });
  });
}

template <auto KernelFunctor, int Dim, typename PropertyList, typename... Args>
inline std::enable_if_t<
    std::is_invocable_v<decltype(KernelFunctor), Args..., char *> &&
        detail::is_property_list_type<PropertyList>::value,
    sycl::event>
...
template <auto KernelFunctor, int Dim, typename... Args>
inline std::enable_if_t<
    std::is_invocable_v<decltype(KernelFunctor), Args..., char *>, sycl::event>
launch(const sycl::nd_range<Dim> &launch_params, std::size_t local_memory_size,
       Args... args) {
...
launch(sycl::nd_range<3> launch_params, const PropertyList &launch_properties,
  ...
  return sycl_exp::submit_with_event(queue, [&](sycl::handler &cgh) {
    sycl_exp::nd_launch(cgh, config,
                        [=](sycl::nd_item<3> it) { KernelFunctor(args...); });
  });
}
...
launch(const dim3 &grid_dim, const dim3 &block_dim,
       const PropertyList &launch_properties, Args... args) {
  return launch<KernelFunctor>(
      sycl::nd_range<3>(grid_dim * block_dim, block_dim), launch_properties,
